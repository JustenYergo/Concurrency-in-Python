...
class SharedCountingDict:
    ...
    def shared_dictionary(self):
        return self.shared_dictionary

    def increment_key_by_value(self, key, value):
        lock = RLock()
        with lock:
            if key not in self.shared_dictionary.keys():
                self.shared_dictionary[key] = value
            if key in self.shared_dictionary.keys():
                self.shared_dictionary[key] += 1
...
if __name__ == "__main__":
    ...
    file_paths = "C:/Users/Justen/OneDrive/Desktop/Lab5/word-count/"  # Location of word-count folder
    # Program Menu
    ...
    for root, directory, files in os.walk(file_paths):
        for file in files:
            ...
            word_counts.append(WordCount(filename, word_counts))
    ...
    for wc in word_counts:
        ...
        # print(f"""Running time for file "{filename}" was {wc.runtime:.3f}s.""")
